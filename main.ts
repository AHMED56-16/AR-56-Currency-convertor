...
console.log(chalk.bgBlueBright('Welcome to the Currency Converter!'));
        ...
        let userAns = await inquirer.prompt([
            {
                name: 'from',
                type: 'list',
                message: chalk.bgYellowBright('Enter From Currency?'),
            ...
            },
            {
                name: 'amount',
                type: 'number',
                message: chalk.redBright('Enter Your desired Amount you want to convert?'),
            }
        ]);
        ...
        let confirm = await inquirer.prompt([
            {
                name: 'toprocess',
                type: "list",
                message: chalk.gray("You want to Continue or Exit?"),
                choices: ['Continue', 'Exit']
            }
        ]);

        if (confirm.toprocess === 'Exit') {
            console.log(chalk.green("Thanks For using CLI Currency Converter"));
            break;
        }
    }
})();